...
public class TestApp2 {
    ...
    private static String maskCipherCVCInPack(String xml) {
        ...
        String buf = xml;
        int startPos = buf.indexOf(MESSAGE_START) + MESSAGE_START.length();
    ...
    private static String maskCipherCVCInMsg(String message) {
        ...
        int startPos = message.toUpperCase().indexOf(MESSAGE_CVV_PREFIX.toUpperCase());
        if (startPos != -1){
            startPos = startPos +  MESSAGE_CVV_PREFIX.length() + 2; //на ": " добавляем 2
